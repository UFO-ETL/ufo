...
// UFO includes.
#include <..\src\script\UFOconsensus.h>
...
int main()
{
    std::cout << "UFOconsensus version: " << UFOconsensus_version() << std::endl;
    ...
    auto op0Result = UFOconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, UFOconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
