#!/usr/bin/env python3
# Copyright (c) 2013-2020 The UFO Core developers
...
def main():
    ...
    # Require service ufo 1.
    ips = [ip for ip in ips if (ip['service'] & 1) == 1]
    print('%s Require service ufo 1' % (ip_stats(ips)), file=sys.stderr)
    ...
    # Filter out hosts with multiple UFO ports, these are likely abusive
    ips = filtermultiport(ips)
    print('%s Filter out hosts with multiple UFO ports' % (ip_stats(ips)), file=sys.stderr)
