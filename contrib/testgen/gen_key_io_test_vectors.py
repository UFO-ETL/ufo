...
import random
from segwit_addr import bech32_encode, decode_segwit_address, convertufos, CHARSET
...
def gen_valid_bech32_vector(template):
    ...
    dst_prefix = bytearray(template[4])
    rv = bech32_encode(hrp, [witver] + convertufos(witprog, 8, 5))
...
def gen_invalid_bech32_vector(template):
    ...
    if no_data:
        ...
        data = [witver] + convertufos(witprog, 8, 5)
