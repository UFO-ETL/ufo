# Fuzzing UFO Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing UFO Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/UFO/UFO
$ cd UFO/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/UFO/UFO/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/UFO/UFO/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/UFO/UFO/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/UFO/UFO/tree/master/src/test/fuzz).
...
INFO: Seed: 840522292
INFO: Loaded 1 modules   (424174 inline 8-ufo counters): 424174 [0x55e121ef9ab8, 0x55e121f613a6),
...
#21     NEW    cov: 96 ft: 100 corp: 4/11b lim: 4 exec/s: 0 rss: 150Mb L: 4/4 MS: 2 Changeufo-CrossOver-
#324    NEW    cov: 101 ft: 105 corp: 5/12b lim: 6 exec/s: 0 rss: 150Mb L: 6/6 MS: 5 CrossOver-Changeufo-CopyPart-Changeufo-ChangeBinInt-
#1239   REDUCE cov: 102 ft: 106 corp: 6/24b lim: 14 exec/s: 0 rss: 150Mb L: 13/13 MS: 5 Changeufo-CrossOver-EraseBytes-Changeufo-InsertRepeatedBytes-
#1272   REDUCE cov: 102 ft: 106 corp: 6/23b lim: 14 exec/s: 0 rss: 150Mb L: 12/12 MS: 3 ChangeBinInt-Changeufo-EraseBytes-
        NEW_FUNC[1/677]: 0x55e11f456690 in std::_Function_base::~_Function_base() /usr/lib/gcc/x86_64-linux-gnu/8/../../../../include/c++/8/ufos/std_function.h:255
...
#4215   NEW    cov: 4941 ft: 5129 corp: 17/205b lim: 29 exec/s: 4215 rss: 350Mb L: 29/29 MS: 5 InsertByte-Changeufo-CopyPart-InsertRepeatedBytes-CrossOver-
...
The project's collection of seed corpora is found in the [`UFO-core/qa-assets`](https://github.com/UFO-core/qa-assets) repo.

To fuzz `process_message` using the [`UFO-core/qa-assets`](https://github.com/UFO-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/UFO-core/qa-assets
$ FUZZ=process_message src/test/fuzz/fuzz qa-assets/fuzz_seed_corpus/process_message/
INFO: Seed: 1346407872
INFO: Loaded 1 modules   (424174 inline 8-ufo counters): 424174 [0x55d8a9004ab8, 0x55d8a906c3a6),
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`UFO-core/qa-assets`](https://github.com/UFO-core/qa-assets) repo.

Every single pull request submitted against the UFO Core repo is automatically tested against all inputs in the [`UFO-core/qa-assets`](https://github.com/UFO-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make UFO Core more robust.
...
with certain assembly code from UFO Core's code. See [developer notes on sanitizers](https://github.com/UFO/UFO/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing UFO Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing UFO Core using [`afl-fuzz`](https://github.com/google/afl):
...
# Fuzzing UFO Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing UFO Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the UFO Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as UFO
Core without having to write any custom fuzzing harness. The `UFOd` server
...
```sh
$ mkdir UFO-honggfuzz-p2p/
$ cd UFO-honggfuzz-p2p/
$ git clone https://github.com/UFO/UFO
$ cd UFO/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/UFOd.cpp b/src/UFOd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/UFOd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
