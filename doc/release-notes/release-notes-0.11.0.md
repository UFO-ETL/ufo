UFO Core version 0.11.0 is now available from:

  <https://UFO.org/bin/UFO-core-0.11.0/>
...
For example, add the following to `UFO.conf`:
...
related to the blockchain in the UFO system: the raw blocks as received over
the network (blk???.dat), the undo data (rev???.dat), the block index and the
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows UFO Core to delete the raw block and undo data once
...
block index and UTXO databases. The minimum was chosen so that UFO Core will
...
- The transaction can then be broadcasted through aruforary mechanisms
  supported by the script

One such application is selective Tor usage, where the node runs on the normal
internet but transactions are broadcasted over Tor.

For an example script see [UFO-submittx](https://github.com/laanwj/UFO-submittx).
...
- #5876 `8e4fd0c` Add a NODE_GETUTXO service ufo and document NODE_NETWORK
...
- #5334 `cf87536` libUFOconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new UFO-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only UFOAmountField
- #5753 `0f44672` Add UFO logo to about screen
...
- #5809 `46bfbe7` Add UFO-cli man page
...
- #5849 `41259ca` contrib/init/UFOd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename UFO-Qt.app to UFO-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for UFO-tx)
