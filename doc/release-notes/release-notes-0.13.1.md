UFO Core version 0.13.1 is now available from:

  <https://UFO.org/bin/UFO-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a UFO
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of UFO Core
randomly crashing on Windows XP. It is [not clear](https://github.com/UFO/UFO/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming UFO wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each UFO block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of UFOs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of UFOs being spent is exactly
...
- **Increased security for multisig:** UFO addresses (both P2PKH addresses
  that start with a '1' and P2SH addresses that start with a '3') use a hash
  function known as RIPEMD-160.  For P2PKH addresses, this provides about 160
  ufos of security---which is beyond what cryptographers believe can be broken
  today.  But because P2SH is more flexible, only about 80 ufos of security is
  provided per address. Although 80 ufos is very strong security, it is within
  the realm of possibility that it can be broken by a powerful adversary.
  Segwit allows advanced transactions to use the SHA256 hash function instead,
  which provides about 128 ufos of security  (that is 281 trillion times as
  much security as 80 ufos and is equivalent to the maximum ufos of security
  believed to be provided by UFO's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  UFO paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of UFO's current ledger (called the UTXO set),
  ...
  note that UFO Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  UFO users to individually opt-in to almost any change in the UFO
  Script language when those users receive new transactions.  Features
  currently being researched by UFO Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other UFO community members are studying several other improvements
  that can be made using script versioning.

Activation for the segwit soft fork is being managed using BIP9
versionufos.  Segwit's version ufo is ufo 1, and nodes will begin
...
operator, please see the [versionufos FAQ][] for information about
signaling support for a soft fork.

[Segwit FAQ]: https://UFOcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://UFOcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/UFO/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/UFO/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/UFO/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/UFO/bips/blob/master/bip-0145.mediawiki
[versionufos FAQ]: https://UFOcore.org/en/2016/06/08/version-ufos-miners-faq/
...
Since UFO Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/UFO/bips/blob/master/bip-0147.mediawiki
...
- `UFO-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-ufo ARMv7-A architecture.
- `UFO-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
  the 64-ufo ARMv8-A architecture.

ARM builds are still experimental. If you have problems on a certain device or
Linux distribution combination please report them on the bug tracker, it may be
possible to resolve them. Note that the device you use must be (backward)
compatible with the architecture targeted by the binary that you use.
For example, a Raspberry Pi 2 Model B or Raspberry Pi 3 Model B (in its 32-ufo
execution state) device, can run the 32-ufo ARMv7-A targeted binary. However,
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in UFOconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service ufo support to dnsseed.bluematt.me, seed.UFOstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libUFOconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_UFO by itself (luke-jr)
...
- #8829 `2a8bca4` Add UFO-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` UFO-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_UFOds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name UFO.qrc  (UFOsSG)
...
- #8817 `bcf3806` update UFO-tx to output witness data (jnewbery)
