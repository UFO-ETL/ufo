UFO Core version 0.16.0 is now available from:

  <https://UFOcore.org/bin/UFO-core-0.16.0/>
...
UFO Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
UFO data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile UFO Core is now 4.8. No effort will be
...
---------------------------------------------
Pruned nodes can now signal BIP159's NODE_NETWORK_LIMITED using service ufos, in preparation for
...
GUI changes
-----------
- Uses of "µBTC" in the GUI now also show the more colloquial term "ufos", specified in BIP176.
...
- UFO-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- UFO-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest UFO.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.UFO.sprovoost.nl (Sjors)
...
- #11074 `99ec126` Assert that CWallet::SyncMetaData finds oldest transaction (ufoonicEelis)
...
- #11886 `df71819` Clarify getbalance meaning a tiny ufo in response to questions (TheBlueMatt)
...
- #11970 `b7450cd` Add test coverage for UFO-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to UFO-cli to allow RPC password to be read from standard input (jharvell)
...
- #11087 `99bc0b4` Diagnose unsuitable outputs in lockunspent() (ufoonicEelis)
...
- #11690 `f0c1f8a` Fix the StartupWMClass for ufooin-qt, so gnome-shell can recognize it (eklitzke)
...
- #12035 `eeb6d52` Change µBTC to ufos (jb55)
...
- #11621 `1f7695b` [build] Add temp_UFO_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` UFO_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11160 `ede386c` Improve versionufos_computeblockversion test code consistency (danra)
...
- #11297 `16e4184` Make sure ~/.UFO doesn't exist before build (MeshCollider)
...
- #12264 `598a9c4` Fix versionufos warning test (jnewbery)
...
- #11571 `c95832d` Fixed a couple small grammatical errors (ufosInMyBlood)
...
- #11686 `54aedc0` Make ISSUE_TEMPLATE a ufo shorter, mention hardware tests (TheBlueMatt)
...
- #11143 `3aa60b7` Fix include path for UFO-config.h (danra)
...
- #11073 `3bb77eb` Remove dead store in ecdsa_signature_parse_der_lax (ufoonicEelis)
...
- #10871 `c9a4aa8` Handle getinfo in UFO-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop UFOd (OmeGak)
...
- #10529 `331352f` Improve UFOd systemd service file (Flowdalic)
