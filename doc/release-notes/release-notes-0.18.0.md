UFO Core version 0.18.0 is now available from:

  <https://UFOcore.org/bin/UFO-core-0.18.0/>
...
`/Applications/UFO-Qt` (on Mac) or `UFOd`/`UFO-qt` (on
Linux).
...
recommended to use UFO Core on unsupported systems.
...
10.10. Additionally, UFO Core does not yet change appearance when
...
`contrib/init/UFOd.service`, it has been changed to use
`/var/lib/UFOd` as the data directory instead of
`~UFO/.UFO`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/UFOd` will
exist has enough space (check using `df -h /var/lib/UFOd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/UFO/UFO/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://UFOcore.org/en/contact/) the UFO Core
...
The systemd init file (`contrib/init/UFOd.service`) has been changed
to use `/var/lib/UFOd` as the data directory instead of
`~UFO/.UFO`. This change makes UFO Core more consistent with
...
`datadir` option in `/etc/UFO/UFO.conf` will have no effect.
...
- A new short [document](https://github.com/UFO/UFO/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/UFO/UFO/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/UFO/UFO/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/UFO/UFO/blob/master/doc/UFO-conf.md)
  about the `UFO.conf` file describes how to use it to configure
  UFO Core.

- A new document introduces UFO Core's BIP174 [Partially-Signed
  UFO Transactions
  (PSBT)](https://github.com/UFO/UFO/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/UFO/UFO/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent UFO-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  UFO Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/UFO/UFO/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/UFO/UFO/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when UFO Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of UFO Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/UFO/UFO/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/UFO/UFO/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart UFOd with the `-deprecatedrpc=generate`
...
- A new `UFO-wallet` tool is now distributed alongside UFO
...
This section describes planned changes to UFO Core that may affect
other UFO software and services.

- Since version 0.16.0, UFO Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with UFO Core 0.20 (expected about
  a year after 0.18), UFO Core will default to native segwit
  ...
  to bech32 addresses, and if the UFO Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in UFO Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any UFO Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, UFO Core
...
- A sub-project of UFO Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with UFO Core.  See their [project
  page](https://github.com/UFO-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to UFO Core's own implementation, although entropy
  gathered by UFO Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves UFO Core a
...
- On macOS, UFO Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `UFO_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libUFO_server.a` (jnewbery)
...
- #13926 [Tools] UFO-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like UFOd (MarcoFalke)
...
- #14521 qt, docs: Fix `UFO-qt -version` output formatting (hebasto)
...
- #11625 Add UFOApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libUFO_server` (laanwj)
...
- #15154 Configure: UFO-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/UFO-wallet (Sjors)
...
- #11526 Visual Studio build configuration for UFO Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libUFO_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in UFO-util-test.py (isghe)
...
- #12775 Integration of property based testing into UFO Core (Christewart)
...
- #12246 Bugfix: Only run UFO-tx tests when UFO-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_UFO` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_UFO` (MarcoFalke)
...
- #14186 UFO-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_UFO.conf` message on startup if conf file exists (leishman)
...
- #12255 Update UFO.service to conform to init.md (dongcarl)
...
- #14153 Add disable-wallet section to OSX build instructions, update line in Unix instructions (ufostein)
...
- #14497 Add doc/UFO-conf.md (hebasto)
...
- #15354 Add missing `UFO-wallet` tool manpages (MarcoFalke)
...
- #15765 correct UFOconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support UFO folder on OSX (JimmyMow)
