UFO Core version 0.19.0.1 is now available from:

  <https://UFOcore.org/bin/UFO-core-0.19.0.1/>
...
UFO Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use UFO Core on unsupported systems.
...
to open the UFO Core .dmg. This is due to new signing requirements
imposed by Apple, which the UFO Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/UFO/UFO/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running UFO Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of UFO
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `UFOd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile UFO Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in UFO Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of UFO Core. (#14054)

- To eliminate well-known denial-of-service vectors in UFO Core,
  ...
  This prevents UFO Core from sending the BIP111 NODE_BLOOM service
  ...
  of UFO Core and nodes that have manually enabled BIP37 support,
...
- By default, UFO Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `UFO-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how UFO Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  UFO Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, UFO Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 UFO address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in UFO Core v0.17.0), this can eliminate a
...
- #16713 Ignore old versionufo activations to avoid 'unknown softforks' warning (jnewbery)
...
- #15649 Add ChaCha20Poly1305@UFO AEAD (jonasschnelli)
...
- #15639 UFO-wallet tool: Drop libUFO_server.a dependency (ryanofsky)
- #15853 Remove unused import checkpoints.h (MarcoFalke)
- #15780 add cachable amounts for caching credit/deufo values (kallewoof)
...
- #16380 Remove unused ufos from the service flags enum (MarcoFalke)
- #16379 Fix autostart filenames on Linux for testnet/regtest (hebasto)
- #16366 init: Use InitError for all errors in UFOd/qt (MarcoFalke)
...
- #15939 gitian: Remove windows 32 ufo build (MarcoFalke)
- #15239 scripts and tools: Move non-linux build source tarballs to "UFO-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of UFOd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from UFO_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_UFO to auto generated project files (sipsorcery)
...
- #15773 Add UFOTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_UFO output (practicalswift)
...
- #15896 feature_filelock, interface_UFO_cli: Use PACKAGE_NAME in messages rather than hardcoding UFO Core (luke-jr)
...
- #13555 parameterize adjustment period in versionufos_computeblockversion (JBaczuk)
...
- #16277 Suppress output in test_UFO for expected errors (gertjaap)
...
- #15840 Contrib scripts: Filter IPv6 by ASN (aufofan)
...
- #16489 log: harmonize UFOd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (UFOVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example UFO.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBUFO_SERVER (Bushstar)
...
- #16080 build/doc: update UFO_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in UFO.conf (fanquake)
...
- #16587 Improve versionufos.h documentation (ariard)
...
- #16621 add default UFO.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the UFO-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, UFO.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/UFOd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/UFO/UFO/).
