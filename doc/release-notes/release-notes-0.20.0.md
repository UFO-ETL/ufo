...
UFO Core version 0.20.0 is now available from:

  <https://UFOcore.org/bin/UFO-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from UFO Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since UFO Core version 0.18.0.
...
- Testing or debugging of implementations of the UFO P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of UFO Core. UFO Core logs debug messages
...
- OpenSSL is no longer used by UFO Core.  (#17265)

- BIP70 support has been fully removed from UFO Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/UFO/UFO/issues/16599),
  [PR #16702](https://github.com/UFO/UFO/pull/16702), and the
  `UFOd help` for more information.  This option is experimental and
...
- All custom settings configured when UFO Core starts are now
...
- The "Start UFO Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://UFOcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple UFO Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed UFO Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/UFO-core/HWI) for
  signing.  Future versions of UFO Core should support a GUI option
...
- UFO Core's automatically-generated source code documentation is
  now available at https://doxygen.UFOcore.org.  (#17596)
...
- The `UFO-cli` utility used with the `-getinfo` parameter now
...
- The `UFO-cli` utility now accepts a `-stdinwalletpassphrase`
...
- The `generatetodescriptor` RPC allows testers using regtest mode to
  generate blocks that pay an aruforary output script descriptor.
  (#16943)
...
- #17068 Always generate `UFOstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_UFO-qt` in msvc build (sipsorcery)
- #17756 Remove `WINDOWS_ufoS` from build system (fanquake)
- #17769 Set `AC_PREREQ` to 2.69 (fanquake)
- #17880 Add -Wdate-time to Werror flags (fanquake)
- #17910 Remove double `LIBUFO_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_UFO` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBUFO_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for UFO-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate UFO schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest UFO.conf (sdaftuar)
...
- #17900 ci: Combine 32-ufo build with centos 7 build (theStack)
...
- #18022 Fix appveyor `test_UFO` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (UFO-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone UFO-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about UFO Core files (hebasto)
...
- #17119 Fix broken UFO-cli examples (andrewtoth)
...
- #17503 Remove ufoness from UFO-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.UFOcore.org (laanwj)
...
- #17648 Rename wallet-tool references to UFO-wallet (hel-o)
...
- #16115 On UFOd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from UFO-cli -getinfo (malevolent)
