...
The UFO Core daemon can be configured to act as a trusted "border
router", implementing the UFO wire protocol and relay, making
...
The ZeroMQ feature in UFO Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building UFOd:
...
For instance:
    ...
    $ UFOd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/UFOd.tx.raw \
...
These options can also be provided in UFO.conf.
...
From the perspective of UFOd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into UFOd directly. Furthermore, no information is
...
using. UFOd appends an up-counting sequence number to each
