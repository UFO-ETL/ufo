...
static void SetupUFOUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " UFO-util utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  UFO-util [options] [commands]  Do stuff\n";
...
static void grind_task(uint32_t nufos, CBlockHeader& header_orig, uint32_t offset, uint32_t step, std::atomic<bool>& found)
{
    arith_uint256 target;
    bool neg, over;
    target.SetCompact(nufos, &neg, &over);
...
static int Grind(int argc, char* argv[], std::string& strPrint)
{
    ...
    uint32_t nufos = header.nufos;
    std::atomic<bool> found{false};

    std::vector<std::thread> threads;
    int n_tasks = std::max(1u, std::thread::hardware_concurrency());
    for (int i = 0; i < n_tasks; ++i) {
        threads.emplace_back( grind_task, nufos, std::ref(header), i, n_tasks, std::ref(found) );
    }
    for (auto& t : threads) {
        t.join();
    }
