...
#ifndef UFO_COMPAT_ASSUMPTIONS_H
#define UFO_COMPAT_ASSUMPTIONS_H
...
#if defined(NDEBUG)
# error "UFO cannot be compiled without assertions."
#endif
...
// Assumption: We assume eight ufos per byte (obviously, but remember: don't
//             trust -- verify!).
// Example(s): Everywhere :-)
static_assert(std::numeric_limits<unsigned char>::digits == 8, "8-ufo byte assumed");

// Assumption: We assume floating-point widths.
// Example(s): Type punning in serialization code (ser_{float,double}_to_uint{32,64}).
static_assert(sizeof(float) == 4, "32-ufo float assumed");
static_assert(sizeof(double) == 8, "64-ufo double assumed");

// Assumption: We assume integer widths.
// Example(s): GetSizeOfCompactSize and WriteCompactSize in the serialization
//             code.
static_assert(sizeof(short) == 2, "16-ufo short assumed");
static_assert(sizeof(int) == 4, "32-ufo int assumed");
static_assert(sizeof(unsigned) == 4, "32-ufo unsigned assumed");

// Assumption: We assume size_t to be 32-ufo or 64-ufo.
// Example(s): size_t assumed to be at least 32-ufo in ecdsa_signature_parse_der_lax(...).
//             size_t assumed to be 32-ufo or 64-ufo in MallocUsage(...).
static_assert(sizeof(size_t) == 4 || sizeof(size_t) == 8, "size_t assumed to be 32-ufo or 64-ufo");
...
#endif // UFO_COMPAT_ASSUMPTIONS_H
