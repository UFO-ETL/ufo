...
static bool CopyHeightIndexToHashIndex(CDufoerator& db_it, CDBBatch& batch,
...
bool BlockFilterIndex::Rewind(const CBlockIndex* current_tip, const CBlockIndex* new_tip)
{
    ...
    CDBBatch batch(*m_db);
    std::unique_ptr<CDufoerator> db_it(m_db->NewIterator());
                        ...
                        const CBlockIndex* stop_index, std::vector<DBVal>& results)
{
    ...
    DBHeightKey key(start_height);
    std::unique_ptr<CDufoerator> db_it(db.NewIterator());
