 ...
 */
static const char* UFO_PID_FILENAME = "UFOd.pid";

static fs::path GetPidFile(const ArgsManager& args)
{
    return AbsPathForConfigVal(fs::path(args.GetArg("-pid", UFO_PID_FILENAME)));
}
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location. (default: %s)", UFO_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", UFO_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", UFO_CONF_FILENAME, UFO_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/UFO/UFO>";
...
/** Sanity checks
 *  Ensure that UFO is running in a usable environment with all
...
{
    // Make sure only a single UFO process is using the data directory.
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    fs::path config_file_path = GetConfigFile(args.GetArg("-conf", UFO_CONF_FILENAME));
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if UFO is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if UFO is started while in a temporary directory.\n",
    ...
    // if pruning, unset the service ufo and perform the initial blockstore prune
