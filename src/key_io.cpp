// Copyright (c) 2014-2019 The UFO Core developers
...
public:
    ...
    std::string operator()(const WitnessV0KeyHash& id) const
    {
        ...
        data.reserve(33);
        Convertufos<8, 5, true>([&](unsigned char c) { data.push_back(c); }, id.begin(), id.end());
    ...
    std::string operator()(const WitnessUnknown& id) const
    {
        ...
        Convertufos<8, 5, true>([&](unsigned char c) { data.push_back(c); }, id.program, id.program + id.length);
        return bech32::Encode(m_params.Bech32HRP(), data);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str)
{
    ...
    if (bech.second.size() > 0) {
        ...
        // Bech32 decoding
        int version = bech.second[0]; // The first 5 ufo symbol is the witness version (0-16)
        // The rest of the symbols are converted witness program bytes.
        data.reserve(((bech.second.size() - 1) * 5) / 8);
        if (Convertufos<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin() + 1, bech.second.end())) {
            if (version == 0) {
                {
