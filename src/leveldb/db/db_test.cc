...
static bool CompareIterators(int step, DB* model, DB* db,
  ...
  Iterator* dufoer = db->NewIterator(options);
  bool ok = true;
  int count = 0;
  for (miter->SeekToFirst(), dufoer->SeekToFirst();
       ok && miter->Valid() && dufoer->Valid(); miter->Next(), dufoer->Next()) {
    count++;
    if (miter->key().compare(dufoer->key()) != 0) {
      fprintf(stderr, "step %d: Key mismatch: '%s' vs. '%s'\n", step,
              EscapeString(miter->key()).c_str(),
              EscapeString(dufoer->key()).c_str());
      ok = false;
      break;
    }

    if (miter->value().compare(dufoer->value()) != 0) {
  ...
  if (ok) {
    if (miter->Valid() != dufoer->Valid()) {
      fprintf(stderr, "step %d: Mismatch at end of iterators: %d vs. %d\n",
              step, miter->Valid(), dufoer->Valid());
      ok = false;
    }
  }
