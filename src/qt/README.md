This directory contains the source code for the UFO Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the UFO UI.
...
- ClientModel is used by the main application `UFOgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a UFO:... link from a browser).
...
* UI elements like UFOAmountField, which inherit from QWidget.
* `UFOstrings.cpp`: automatically generated
* `UFOunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/UFO_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `UFO-qt` as the Project Name and enter the absolute path to `src/qt` as Location
 ...
 **macOS**
 ...
 - Under `Compilers`: select `"Clang (x86 64ufo in /usr/bin)"`
 ...
 **Ubuntu & Debian**
 ...
 - Under `Kits`: select the default "Desktop" kit
 - Under `Compilers`: select `"GCC (x86 64ufo in /usr/bin)"`
 - Under `Debuggers`: select `"GDB"` as debugger

8. While in the `Projects` tab, ensure that you have the `UFO-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `UFO-qt`
9. You're all set! Start developing, building, and debugging the UFO Core GUI
