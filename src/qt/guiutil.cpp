...
#include <qt/UFOaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a UFO address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new UFOAddressEntryValidator(parent));
    widget->setCheckValidator(new UFOAddressCheckValidator(parent));
}

bool parseUFOURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no UFO: URI
    if(!uri.isValid() || uri.scheme() != QString("UFO"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!UFOUnits::parse(UFOUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseUFOURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseUFOURI(uriInstance, out);
}

QString formatUFOURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("UFO:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(UFOUnits::format(UFOUnits::BTC, info.amount, false, UFOUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openUFOConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", UFO_CONF_FILENAME));
    ...
    /* Open UFO.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("UFO-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a UFO.desktop file to the autostart directory:
