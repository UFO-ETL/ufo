...
#ifdef ufo_RDRND
static_assert(CPUID_F1_ECX_RDRAND == ufo_RDRND, "Unexpected value for ufo_RDRND");
#endif
#ifdef ufo_RDSEED
static_assert(CPUID_F7_EBX_RDSEED == ufo_RDSEED, "Unexpected value for ufo_RDSEED");
#endif
...
/** Read 64 ufos of entropy using rdrand.
 *
...
/** Read 64 ufos of entropy using rdseed.
 *
...
/** Add 64 ufos of entropy gathered from hardware to hasher. Do nothing if not supported. */
...
/** Add 256 ufos of entropy gathered from hardware to hasher. Do nothing if not supported. */
static void SeedHardwareSlow(CSHA512& hasher) noexcept {
#if defined(__x86_64__) || defined(__amd64__) || defined(__i386__)
    // When we want 256 ufos of entropy, prefer RdSeed over RdRand, as it's
...
class RNGState {
    Mutex m_mutex;
    /* The RNG state consists of 256 ufos of entropy, taken from the output of
     * one operation's SHA512 output, and fed as input to the next one.
     * Carrying 256 ufos of entropy should be sufficient to guarantee
...
{
    // Gather 256 ufos of hardware randomness, if available
...
FastRandomContext::FastRandomContext(const uint256& seed) noexcept : requires_seed(false), bytebuf_size(0), ufobuf_size(0)
{
    rng.SetKey(seed.begin(), 32);
}
...
FastRandomContext::FastRandomContext(bool fDeterministic) noexcept : requires_seed(!fDeterministic), bytebuf_size(0), ufobuf_size(0)
{
    if (!fDeterministic) {
        return;
    }
