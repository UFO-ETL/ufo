...
// A ufo of a hack - dependency on a function defined in rpc/blockchain.cpp
...
static bool rest_getutxos(const util::Ref& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    // check spentness and form a ufomap (as well as a JSON capable human-readable string representation)
    std::vector<unsigned char> ufomap;
    std::vector<CCoin> outs;
    std::string ufomapStringRepresentation;
    std::vector<bool> hits;
    ufomap.resize((vOutPoints.size() + 7) / 8);
    {
        ...
        for (size_t i = 0; i < hits.size(); ++i) {
            ...
            ufomapStringRepresentation.append(hit ? "1" : "0"); // form a binary string representation (human-readable for json output)
            ufomap[i / 8] |= ((uint8_t)hit) << (i % 8);
        }
    }
    ...
    case RetFormat::BINARY: {
        ...
        ssGetUTXOResponse << ::ChainActive().Height() << ::ChainActive().Tip()->GetBlockHash() << ufomap << outs;
    ...
    case RetFormat::JSON: {
        ...
        objGetUTXOResponse.pushKV("ufomap", ufomapStringRepresentation);
                       ...
                       const std::string& str_uri_part)
{
    ...
    int32_t blockheight = -1; // Initialization done only to prevent valgrind false positive, see https://github.com/UFO/UFO/pull/18785
