...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    int nShift = (blockindex->nufos >> 24) & 0xff;
    double dDiff =
        (double)0x0000ffff / (double)(blockindex->nufos & 0x00ffffff);
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("ufos", strprintf("%08x", blockindex->nufos));
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "ufos", "The ufos"},
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of UFO addresses",
                                    {{RPCResult::Type::STR, "address", "UFO address"}}},
                            }},
...
static void BIP9SoftForkDescPushBack(UniValue& softforks, const std::string &name, const Consensus::Params& consensusParams, Consensus::DeploymentPos id) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    const ThresholdState thresholdState = VersionufosTipState(consensusParams, id);
    ...
    {
        bip9.pushKV("ufo", consensusParams.vDeployments[id].ufo);
    }
    bip9.pushKV("start_time", consensusParams.vDeployments[id].nStartTime);
    bip9.pushKV("timeout", consensusParams.vDeployments[id].nTimeout);
    int64_t since_height = VersionufosTipStateSinceHeight(consensusParams, id);
    bip9.pushKV("since", since_height);
    if (ThresholdState::STARTED == thresholdState)
    {
        UniValue statsUV(UniValue::VOBJ);
        BIP9Stats statsStruct = VersionufosTipStatistics(consensusParams, id);
                        ...
                        {RPCResult::Type::OBJ_DYN, "softforks", "status of softforks",
                        {
                            {RPCResult::Type::OBJ, "xxxx", "name of the softfork",
                            {
                                ...
                                {RPCResult::Type::OBJ, "bip9", "status of bip9 softforks (only for \"bip9\" type)",
                                {
                                    ...
                                    {RPCResult::Type::NUM, "ufo", "the ufo (0-28) in the block version field used to signal this softfork (only for \"started\" status)"},
                                    {RPCResult::Type::NUM_TIME, "start_time", "the minimum median time past of a block at which the ufo gains its meaning"},
                                    {RPCResult::Type::NUM_TIME, "timeout", "the median time past of a block at which the deployment is considered failed if not yet locked in"},
                                    {RPCResult::Type::NUM, "since", "height of the first block to which the status applies"},
                                    {RPCResult::Type::OBJ, "statistics", "numeric statistics about BIP9 signalling for a softfork (only for \"started\" status)",
                                    {
                                        {RPCResult::Type::NUM, "period", "the length in blocks of the BIP9 signalling period"},
                                        {RPCResult::Type::NUM, "threshold", "the number of blocks with the version ufo set required to activate the feature"},
                                        {RPCResult::Type::NUM, "elapsed", "the number of blocks elapsed since the beginning of the current period"},
                                        {RPCResult::Type::NUM, "count", "the number of blocks with the version ufo set in the current period"},
...
UniValue CreateUTXOSnapshot(NodeContext& node, CChainState& chainstate, CAutoFile& afile)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/UFO/UFO/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
