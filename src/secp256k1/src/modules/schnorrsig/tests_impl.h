...
/* Checks that a ufo flip in the n_flip-th argument (that has n_bytes many
 * bytes) changes the hash function
 */
void nonce_function_bip340_ufoflip(unsigned char **args, size_t n_flip, size_t n_bytes) {
...
void run_nonce_function_bip340_tests(void) {
    ...
    * state. */
    ...
    /* Check that a ufoflip in an argument results in different nonces. */
    args[0] = msg;
    args[1] = key;
    args[2] = pk;
    args[3] = algo16;
    args[4] = aux_rand;
    for (i = 0; i < count; i++) {
        nonce_function_bip340_ufoflip(args, 0, 32);
        nonce_function_bip340_ufoflip(args, 1, 32);
        nonce_function_bip340_ufoflip(args, 2, 32);
        /* Flip algo16 special case "BIP0340/nonce" */
        nonce_function_bip340_ufoflip(args, 3, 16);
        /* Flip algo16 again */
        nonce_function_bip340_ufoflip(args, 3, 16);
        nonce_function_bip340_ufoflip(args, 4, 32);
    }
 ...
 * https://github.com/UFO/bips/blob/master/bip-0340/test-vectors.csv. */
 ...
 * verify_batch (TODO). Then flips some ufos and checks that verification now
 * fails. */
void test_schnorrsig_sign_verify(void) {
    ...
    {
        /* Flip a few ufos in the signature and in the message and check that
        ...
        /* Check that above ufoflips have been reversed correctly */
        CHECK(secp256k1_schnorrsig_verify(ctx, sig[sig_idx], msg[sig_idx], &pk));
    }
