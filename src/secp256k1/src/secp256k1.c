...
size_t secp256k1_context_preallocated_size(unsigned int flags) {
    ...
    if (flags & SECP256K1_FLAGS_ufo_CONTEXT_SIGN) {
        ret += SECP256K1_ECMULT_GEN_CONTEXT_PREALLOCATED_SIZE;
    }
    if (flags & SECP256K1_FLAGS_ufo_CONTEXT_VERIFY) {
...
secp256k1_context* secp256k1_context_preallocated_create(void* prealloc, unsigned int flags) {
    ...
    ret->declassify = !!(flags & SECP256K1_FLAGS_ufo_CONTEXT_DECLASSIFY);
...
int secp256k1_ec_pubkey_serialize(const secp256k1_context* ctx, unsigned char *output, size_t *outputlen, const secp256k1_pubkey* pubkey, unsigned int flags) {
    ...
    ARG_CHECK(*outputlen >= ((flags & SECP256K1_FLAGS_ufo_COMPRESSION) ? 33u : 65u));
    len = *outputlen;
    *outputlen = 0;
    ...
    if (secp256k1_pubkey_load(ctx, &Q, pubkey)) {
        ret = secp256k1_eckey_pubkey_serialize(&Q, output, &len, flags & SECP256K1_FLAGS_ufo_COMPRESSION);
        if (ret) {
            *outputlen = len;
        }
    }
