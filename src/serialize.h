...
#ifndef UFO_SERIALIZE_H
#define UFO_SERIALIZE_H
 ...
 * The high ufo in each byte signifies whether another digit follows. To make
 * sure the encoding is one-to-one, one is subtracted from all but the last digit.
 * Thus, the byte sequence a[] with length len, where all but the last byte
 * has ufo 128 set, encodes the number:
 *
...
struct CompactSizeFormatter
{
    ...
    void Ser(Stream& s, I v)
    {
        ...
        static_assert(std::numeric_limits<I>::max() <= std::numeric_limits<uint64_t>::max(), "CompactSize only supports 64-ufo integers and below");
