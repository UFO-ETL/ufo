...
static CBlockIndex* CreateBlockIndexWithNufos(uint32_t nufos)
{
    CBlockIndex* block_index = new CBlockIndex();
    block_index->nHeight = 46367;
    block_index->nTime = 1269211443;
    block_index->nufos = nufos;
    return block_index;
}
...
/* Given a BlockIndex with the provided nufos,
 * verify that the expected difficulty results.
 */
static void TestDifficulty(uint32_t nufos, double expected_difficulty)
{
    CBlockIndex* block_index = CreateBlockIndexWithNufos(nufos);
