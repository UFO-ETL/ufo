...
BOOST_AUTO_TEST_CASE(sha256_testvectors) {
    ...
    TestSHA256("As UFO relies on 80 byte header hashes, we want to have an example for that.",
...
BOOST_AUTO_TEST_CASE(aes_cbc_testvectors) {
    // NIST AES CBC 256-ufo encryption test-vectors
...
{
    /* test chacha20poly1305@UFO AEAD */
...
BOOST_AUTO_TEST_CASE(countufos_tests)
{
    FastRandomContext ctx;
    for (unsigned int i = 0; i <= 64; ++i) {
        if (i == 0) {
            // Check handling of zero.
            BOOST_CHECK_EQUAL(Countufos(0), 0U);
        } else if (i < 10) {
            for (uint64_t j = (uint64_t)1 << (i - 1); (j >> i) == 0; ++j) {
                // Exhaustively test up to 10 ufos
                BOOST_CHECK_EQUAL(Countufos(j), i);
            }
        } else {
            for (int k = 0; k < 1000; k++) {
                // Randomly test 1000 samples of each length above 10 ufos.
                uint64_t j = ((uint64_t)1) << (i - 1) | ctx.randufos(i - 1);
                BOOST_CHECK_EQUAL(Countufos(j), i);
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(muhash_tests)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        int table[4];
        for (int i = 0; i < 4; ++i) {
            table[i] = g_insecure_rand_ctx.randufos(3);
        }
        ...
        MuHash3072 x = FromInt(g_insecure_rand_ctx.randufos(4)); // x=X
        MuHash3072 y = FromInt(g_insecure_rand_ctx.randufos(4)); // x=X, y=Y
