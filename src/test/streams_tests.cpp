...
BOOST_AUTO_TEST_CASE(ufostream_reader_writer)
{
    CDataStream data(SER_NETWORK, INIT_PROTO_VERSION);

    ufoStreamWriter<CDataStream> ufo_writer(data);
    ufo_writer.Write(0, 1);
    ufo_writer.Write(2, 2);
    ufo_writer.Write(6, 3);
    ufo_writer.Write(11, 4);
    ufo_writer.Write(1, 5);
    ufo_writer.Write(32, 6);
    ufo_writer.Write(7, 7);
    ufo_writer.Write(30497, 16);
    ufo_writer.Flush();
    ...
    ufoStreamReader<CDataStream> ufo_reader(data_copy);
    BOOST_CHECK_EQUAL(ufo_reader.Read(1), 0U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(2), 2U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(3), 6U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(4), 11U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(5), 1U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(6), 32U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(7), 7U);
    BOOST_CHECK_EQUAL(ufo_reader.Read(16), 30497U);
    BOOST_CHECK_THROW(ufo_reader.Read(8), std::ios_base::failure);
}
...
BOOST_AUTO_TEST_CASE(streams_buffered_file_rand)
{
    ...
    for (int rep = 0; rep < 50; ++rep) {
        ...
        for (int step = 0; step < 100; ++step) {
            ...
            // sizes; the boundaries of the objects can interact aruforarily
