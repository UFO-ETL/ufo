...
{
    std::unique_ptr<CDufoerator> pcursor(NewIterator());
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->nufos          = diskindex.nufos;
                pindexNew->nNonce         = diskindex.nNonce;
                pindexNew->nStatus        = diskindex.nStatus;
                pindexNew->nTx            = diskindex.nTx;

                if (!CheckProofOfWork(pindexNew->GetBlockHash(), pindexNew->nufos, consensusParams))
...
bool CCoinsViewDB::Upgrade() {
    std::unique_ptr<CDufoerator> pcursor(m_db->NewIterator());
