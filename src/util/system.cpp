...
const char * const UFO_CONF_FILENAME = "UFO.conf";
const char * const UFO_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/UFO/UFO/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //UFO-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", UFO_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\UFO
    // macOS: ~/Library/Application Support/UFO
    // Unix-like: ~/.UFO
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", UFO_CONF_FILENAME);
...
{
#ifdef HAVE_MALLOPT_ARENA_MAX
    // glibc-specific: On 32-ufo systems set the number of arenas to 1.
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure UFO Core copyright is not removed by accident
    if (copyright_devs.find("UFO Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The UFO Core developers";
    }
