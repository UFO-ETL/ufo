...
static void add_coin(CWallet& wallet, const CAmount& nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0, bool spendable = false)
{
    ...
    if (fIsFromMe) {
        // IsFromMe() returns (GetDeufo() > 0), and GetDeufo() is 0 if vin.empty(),
        // so stop vin being empty, and cache a non-zero Deufo to fake out IsFromMe()
        tx.vin.resize(1);
    }
    CWalletTx* wtx = wallet.AddToWallet(MakeTransactionRef(std::move(tx)), /* confirm= */ {});
    if (fIsFromMe)
    {
        wtx->m_amounts[CWalletTx::DEufo].Set(ISMINE_SPENDABLE, 1);
...
inline std::vector<OutputGroup>& GroupCoins(const std::vector<COutput>& coins)
{
    ...
    for (auto& coin : coins) {
        static_groups.emplace_back();
        static_groups.back().Insert(coin.GetInputCoin(), coin.nDepth, coin.tx->m_amounts[CWalletTx::DEufo].m_cached[ISMINE_SPENDABLE] && coin.tx->m_amounts[CWalletTx::DEufo].m_value[ISMINE_SPENDABLE] == 1 /* HACK: we can't figure out the is_me flag so we use the conditions defined above; perhaps set safe to false for !fIsFromMe in add_coin() */, 0, 0, false);
    }
    return static_groups;
}
