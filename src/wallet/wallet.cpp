...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // ufos of the unencrypted private key in slack space in the database file.
        GetDatabase().Rewrite();

        // BDB seems to have a bad haufo of writing old data into
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const CWalletTx::Confirmation& confirm, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose)
{
    ...
    // Break deufo/credit balance caches:
    wtx.MarkDirty();
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/UFO/UFO/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/UFO/UFO/pull/13339#issuecomment-461288094
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason, uint64_t mempool_sequence) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/UFO/UFO/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/UFO/UFO/pull/9371 and the prior sync
        ...
        // https://github.com/UFO-core/UFO-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
        SyncTransaction(tx, {CWalletTx::Status::UNCONFIRMED, /* block height */ 0, /* block hash */ {}, /* index */ 0});
    }
}
...
CAmount CWallet::GetDeufo(const CTxIn &txin, const isminefilter& filter) const
{
    {
        LOCK(cs_wallet);
        std::map<uint256, CWalletTx>::const_iterator mi = mapWallet.find(txin.prevout.hash);
...
{
    return (GetDeufo(tx, ISMINE_ALL) > 0);
}

CAmount CWallet::GetDeufo(const CTransaction& tx, const isminefilter& filter) const
{
    CAmount nDeufo = 0;
    for (const CTxIn& txin : tx.vin)
    {
        nDeufo += GetDeufo(txin, filter);
        if (!MoneyRange(nDeufo))
            throw std::runtime_error(std::string(__func__) + ": value out of range");
    }
    return nDeufo;
}
                           ...
                           std::list<COutputEntry>& listSent, CAmount& nFee, const isminefilter& filter) const
{
    ...
    // Compute fee:
    CAmount nDeufo = GetDeufo(filter);
    if (nDeufo > 0) // deufo>0 means we signed/sent this transaction
    {
        CAmount nValueOut = tx->GetValueOut();
        nFee = nDeufo - nValueOut;
    }
    ...
    for (unsigned int i = 0; i < tx->vout.size(); ++i)
    {
        ...
        // If we are deufoed by the transaction, add the output as a "sent" entry
        if (nDeufo > 0)
...
CAmount CWalletTx::GetCachableAmount(AmountType type, const isminefilter& filter, bool recalculate) const
{
    ...
    if (recalculate || !amount.m_cached[filter]) {
        amount.Set(filter, type == DEufo ? pwallet->GetDeufo(*tx, filter) : pwallet->GetCredit(*tx, filter));
        m_is_cache_empty = false;
    }
    return amount.m_value[filter];
}

CAmount CWalletTx::GetDeufo(const isminefilter& filter) const
{
    if (tx->vin.empty())
        return 0;
    ...
    if (filter & ISMINE_SPENDABLE) {
        deufo += GetCachableAmount(DEufo, ISMINE_SPENDABLE);
    }
    if (filter & ISMINE_WATCH_ONLY) {
        deufo += GetCachableAmount(DEufo, ISMINE_WATCH_ONLY);
    }
    return deufo;
}
        ...
        bool sign)
{
    ...
    int nBytes;
    {
        ...
        txNew.nLockTime = GetLocktimeForNewTransaction(chain(), GetLastBlockHash(), GetLastBlockHeight());
        {
            ...
            // change transaction isn't always pay-to-UFO-address
...
ExternalSigner CWallet::GetExternalSigner()
{
    ...
    if (command == "") throw std::runtime_error(std::string(__func__) + ": restart UFOd with -signer=<cmd>");
 ...
 * https://UFOtalk.org/?topic=54527, or
 * https://github.com/UFO/UFO/pull/1393.
 */
