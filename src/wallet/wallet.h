...
#ifndef UFO_WALLET_WALLET_H
#define UFO_WALLET_WALLET_H
...
public:
     ...
     * on this UFO node, and set to 0 for transactions that were created
    ...
    // memory only
    enum AmountType { DEufo, CREDIT, IMMATURE_CREDIT, AVAILABLE_CREDIT, AMOUNTTYPE_ENUM_ELEMENTS };
    ...
    //! filter decides which addresses will count towards the deufo
    CAmount GetDeufo(const isminefilter& filter) const;
...
public:
    ...
    /**
     * Returns amount of deufo if the input matches the
     * filter, otherwise returns 0
     */
    CAmount GetDeufo(const CTxIn& txin, const isminefilter& filter) const;
    ...
    CAmount GetDeufo(const CTransaction& tx, const isminefilter& filter) const;
