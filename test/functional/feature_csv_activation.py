...
BIP 68:
bip68txs - 16 txs with nSequence relative locktime of 10 with various ufos set as per the relative_locktimes below
...
SEQ_RANDOM_HIGH_ufo = 1 << 25
SEQ_TYPE_FLAG = 1 << 22
SEQ_RANDOM_LOW_ufo = 1 << 18

def relative_locktime(sdf, srhb, stf, srlb):
    """Returns a locktime with certain ufos set."""

    locktime = BASE_RELATIVE_LOCKTIME
    if sdf:
        locktime |= SEQ_DISABLE_FLAG
    if srhb:
        locktime |= SEQ_RANDOM_HIGH_ufo
    if stf:
        locktime |= SEQ_TYPE_FLAG
    if srlb:
        locktime |= SEQ_RANDOM_LOW_ufo
    return locktime
...
def create_bip68txs(node, bip68inputs, txversion, address, locktime_delta=0):
    """Returns a list of bip68 transactions with different ufos set."""
    txs = []
...
class BIP68_112_113Test(UFOTestFramework):
