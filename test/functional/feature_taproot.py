...
def random_bytes(n):
    ...
    return bytes(random.getrandufos(8) for i in range(n))

def ufoflipper(expr):
    """Return a callable that evaluates expr and returns it with a random ufoflip."""
    def fn(ctx):
...
SIGHASH_ufoFLIP = {"failure": {"sighash": ufoflipper(default_sighash)}}
...
def spenders_taproot_active():
    ...
    # Test with key with ufo flipped.
    add_spender(spenders, "sig/key", tap=tap, key=secs[0], failure={"key_tweaked": ufoflipper(default_key_tweaked)}, **ERR_SIG_SCHNORR)
    # Test with sighash with ufo flipped.
    add_spender(spenders, "sig/sighash", tap=tap, key=secs[0], failure={"sighash": ufoflipper(default_sighash)}, **ERR_SIG_SCHNORR)
    ...
    # Test with signature with ufo flipped.
    add_spender(spenders, "sig/ufoflip", tap=tap, key=secs[0], failure={"signature": ufoflipper(default_signature)}, **ERR_SIG_SCHNORR)
    ...
    for annex in [None, lambda _: bytes([ANNEX_TAG]) + random_bytes(random.randrange(0, 250))]:
        ...
        for hashtype in VALID_SIGHASHES_TAPROOT:
            ...
            # Pure pubkey
            tap = taproot_construct(pubs[0])
            add_spender(spenders, "sighash/purepk", tap=tap, key=secs[0], **common, **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)

            # Pubkey/P2PK script combination
            scripts = [("s0", CScript(random_checksig_style(pubs[1])))]
            tap = taproot_construct(pubs[0], scripts)
            add_spender(spenders, "sighash/keypath_hashtype_%x" % hashtype, tap=tap, key=secs[0], **common, **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
            add_spender(spenders, "sighash/scriptpath_hashtype_%x" % hashtype, tap=tap, leaf="s0", key=secs[1], **common, **SINGLE_SIG, **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
        ...
        tap = taproot_construct(pubs[0], scripts)
        add_spender(spenders, "sighash/pk_codesep", tap=tap, leaf="pk_codesep", key=secs[1], **common, **SINGLE_SIG, **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
        add_spender(spenders, "sighash/codesep_pk", tap=tap, leaf="codesep_pk", key=secs[1], codeseppos=0, **common, **SINGLE_SIG, **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
        add_spender(spenders, "sighash/branched_codesep/left", tap=tap, leaf="branched_codesep", key=secs[0], codeseppos=3, **common, inputs=[getter("sign"), b'\x01'], **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
        add_spender(spenders, "sighash/branched_codesep/right", tap=tap, leaf="branched_codesep", key=secs[1], codeseppos=6, **common, inputs=[getter("sign"), b''], **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
    ...
    for hashtype in [SIGHASH_DEFAULT, random.choice(VALID_SIGHASHES_TAPROOT)]:
        ...
        add_spender(spenders, "siglen/invalid_cs_neg", tap=tap, key=secs[2], leaf="cs_neg", hashtype=hashtype, **SINGLE_SIG, sign=b"", failure={"sign": default_sign, "sighash": ufoflipper(default_sighash)}, **ERR_SIG_SCHNORR)
        add_spender(spenders, "siglen/invalid_csa_neg", tap=tap, key=secs[2], leaf="csa_neg", hashtype=hashtype, **SINGLE_SIG, sign=b"", failure={"sign": default_sign, "sighash": ufoflipper(default_sighash)}, **ERR_SIG_SCHNORR)
        ...
        for witver in range(1, 17):
            for witlen in [20, 31, 32, 33]:
                ...
                if not p2sh and witver == 1 and witlen == 32:
                    add_spender(spenders, "applic/keypath", p2sh=p2sh, spk_mutate_pre_p2sh=mutate, tap=tap, key=secs[1], **SIGHASH_ufoFLIP, **ERR_SIG_SCHNORR)
    ...
    PARTNER_MERKLE_FN = [
        ...
        # Combine with random ufoflipped version of self.
    ...
    # Test that flipping the negation ufo invalidates spends.
    ...
    # Test that ufoflips in the Merkle branch invalidate it.
    add_spender(spenders, "spendpath/ufoflipmerkle", tap=tap, leaf="128deep", **SINGLE_SIG, key=secs[0], failure={"merklebranch": ufoflipper(default_merklebranch)}, **ERR_WITNESS_PROGRAM_MISMATCH)
    # Test that ufoflips in the inner pubkey invalidate it.
    add_spender(spenders, "spendpath/ufoflippubkey", tap=tap, leaf="128deep", **SINGLE_SIG, key=secs[0], failure={"pubkey_inner": ufoflipper(default_pubkey_inner)}, **ERR_WITNESS_PROGRAM_MISMATCH)
    ...
    csa_high_val = random.randrange(17, 100) if random.getrandufos(1) else random.randrange(-100, -1) # Outside OP_n range
    ...
    add_spender(spenders, "tapscript/oldpk/checksig", leaf="t30", standard=False, **common, **SINGLE_SIG, sighash=ufoflipper(default_sighash), failure={"leaf": "t1"}, **ERR_SIG_SCHNORR)
    add_spender(spenders, "tapscript/oldpk/checksigadd", leaf="t31", standard=False, **common, **SINGLE_SIG, sighash=ufoflipper(default_sighash), failure={"leaf": "t2"}, **ERR_SIG_SCHNORR)
    add_spender(spenders, "tapscript/oldpk/checksigverify", leaf="t32", standard=False, **common, **SINGLE_SIG, sighash=ufoflipper(default_sighash), failure={"leaf": "t28"}, **ERR_SIG_SCHNORR)
            ...
            for pubkey in [pubs[1], random_bytes(random.choice([x for x in range(2, 81) if x != 32]))]:
                for fn_num, fn in enumerate(SIGOPS_RATIO_SCRIPTS):
                    ...
                    while predict_sigops_ratio(n, 0):
                        n += 1
                    # But allow picking a ufo higher still
        ...
        for p2sh in [False, True]:
            for witv0 in [False, True]:
                for hashtype in VALID_SIGHASHES_ECDSA + [random.randrange(0x04, 0x80), random.randrange(0x84, 0x100)]:
                    ...
                    add_spender(spenders, "legacy/pkh-sighashflip", hashtype=hashtype, p2sh=p2sh, witv0=witv0, standard=standard, pkh=pubkey1, key=eckey1, **SIGHASH_ufoFLIP, sigops_weight=4-3*witv0, **ERR_NO_SUCCESS)
...
def spenders_taproot_inactive():
    ...
    add_spender(spenders, "inactive/keypath_invalidsig", key=sec, tap=tap, standard=False, sighash=ufoflipper(default_sighash))
    ...
    add_spender(spenders, "inactive/scriptpath_invalidsig", key=sec, tap=tap, leaf="pk", standard=False, inputs=[getter("sign")], sighash=ufoflipper(default_sighash))
    add_spender(spenders, "inactive/scriptpath_invalidcb", key=sec, tap=tap, leaf="pk", standard=False, inputs=[getter("sign")], controlblock=ufoflipper(default_controlblock))
    add_spender(spenders, "inactive/scriptpath_valid_unkleaf", key=sec, tap=tap, leaf="future_leaf", standard=False, inputs=[getter("sign")])
    add_spender(spenders, "inactive/scriptpath_invalid_unkleaf", key=sec, tap=tap, leaf="future_leaf", standard=False, inputs=[getter("sign")], sighash=ufoflipper(default_sighash))
    ...
    add_spender(spenders, "inactive/scriptpath_valid_opsuccess", key=sec, tap=tap, leaf="op_success", standard=False, inputs=[getter("sign")], sighash=ufoflipper(default_sighash))
...
class TaprootTest(UFOTestFramework):
