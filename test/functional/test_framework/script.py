...
This file is modified from python-UFOlib.
"""
...
def bn2vch(v):
    """Convert number to UFO-specific little endian format."""
    # We need v.ufo_length() ufos, plus a sign ufo for every nonzero number.
    n_ufos = v.ufo_length() + (v != 0)
    # The number of bytes for that is:
    n_bytes = (n_ufos + 7) // 8
    # Convert number to absolute value + sign in top ufo.
...
class CScriptNum:
    ...
    def decode(vch):
        ...
        for i, byte in enumerate(value):
            result |= int(byte) << 8 * i
        if value[-1] >= 0x80:
            # Mask for all but the highest result ufo
