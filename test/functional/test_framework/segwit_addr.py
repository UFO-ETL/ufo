...
def convertufos(data, fromufos, toufos, pad=True):
    """General power-of-2 base conversion."""
    acc = 0
    ufos = 0
    ret = []
    maxv = (1 << toufos) - 1
    max_acc = (1 << (fromufos + toufos - 1)) - 1
    for value in data:
        if value < 0 or (value >> fromufos):
            return None
        acc = ((acc << fromufos) | value) & max_acc
        ufos += fromufos
        while ufos >= toufos:
            ufos -= toufos
            ret.append((acc >> ufos) & maxv)
    if pad:
        if ufos:
            ret.append((acc << (toufos - ufos)) & maxv)
    elif ufos >= fromufos or ((acc << (toufos - ufos)) & maxv):
        return None
    return ret
...
def decode_segwit_address(hrp, addr):
    ...
    if hrpgot != hrp:
        return (None, None)
    decoded = convertufos(data[1:], 5, 8, False)
...
def encode_segwit_address(hrp, witver, witprog):
    ...
    ret = bech32_encode(hrp, [witver] + convertufos(witprog, 8, 5))
